...
mae_CC_in_ADD_100 = []
# R^2
r_squared_HFDM_in_ADD_100 = []
r_squared_AC_in_ADD_100 = []
r_squared_MC_in_ADD_100 = []
r_squared_CC_in_ADD_100 = []
# ME
me_HFDM_in_ADD_100 = []
me_AC_in_ADD_100 = []
me_MC_in_ADD_100 = []
...
for i in range(100):
    ...
    AC_best_alpha_in_ADD, MC_best_alpha_in_ADD, CC_best_alpha_in_ADD, HFDM_best_alpha_in_ADD = None, None, None, None
    AC_best_mse_in_ADD, MC_best_mse_in_ADD, CC_best_mse_in_ADD, HFDM_best_mse_in_ADD = float('inf'), float('inf'), float('inf'), float('inf')
    ...
    Yh = [y_true(e) + 0.4 * random.random() * random.choice([-1, 1]) for e in Xh]
    Yl = [y_true(e) + 0.8 * random.random() * random.choice([-1, 1]) for e in Xl]
    ...
    for alpha in alpha_values:
        ...
        AC_5fold_in_ADD = []  # 5fold 中每次在加性噪声的数据集下得到的mse
        MC_5fold_in_ADD = []
        CC_5fold_in_ADD = []
        HFDM_5fold_in_ADD = []
        ...
        for k in range(5):
            ...
            AC_5fold_in_ADD.append(mse_AC_in_add)
            mse_MC_in_add = mean_squared_error(Y_val_add, MC_in_add_pred)
            MC_5fold_in_ADD.append(mse_MC_in_add)
            mse_CC_in_add = mean_squared_error(Y_val_add, CC_in_add_pred)
            CC_5fold_in_ADD.append(mse_CC_in_add)
            mse_HFDM_in_add = mean_squared_error(Y_val_add, HFDM_in_add_pred)
            HFDM_5fold_in_ADD.append(mse_HFDM_in_add)


        # 计算在当前参数alpha下的5折平均mse
        AC_val_avg_mse = np.mean(AC_5fold_in_ADD)
        MC_val_avg_mse = np.mean(MC_5fold_in_ADD)
        CC_val_avg_mse = np.mean(CC_5fold_in_ADD)
        HFDM_val_avg_mse = np.mean(HFDM_5fold_in_ADD)
        ...
        # 记录最佳参数和mse
        if AC_val_avg_mse < AC_best_mse_in_ADD:
            AC_best_mse_in_ADD = AC_val_avg_mse
            AC_best_alpha_in_ADD = alpha
        if MC_val_avg_mse < MC_best_mse_in_ADD:
            MC_best_mse_in_ADD = MC_val_avg_mse
            MC_best_alpha_in_ADD = alpha
        if CC_val_avg_mse < CC_best_mse_in_ADD:
            CC_best_mse_in_ADD = CC_val_avg_mse
            CC_best_alpha_in_ADD = alpha
        if HFDM_val_avg_mse < HFDM_best_mse_in_ADD:
            HFDM_best_mse_in_ADD = HFDM_val_avg_mse
            HFDM_best_alpha_in_ADD = alpha

    print("AC MC CC HFDM 的最佳alpha超参数:", AC_best_alpha_in_ADD, MC_best_alpha_in_ADD, CC_best_alpha_in_ADD, HFDM_best_alpha_in_ADD)

    # 使用最佳参数alpha去训练模型
    best_AC = AC(Xh, Xl, Yh, Yl, AC_best_alpha_in_ADD)
    best_MC = MC(Xh, Xl, Yh, Yl, MC_best_alpha_in_ADD)
    best_CC = CC(Xh, Xl, Yh, Yl, CC_best_alpha_in_ADD)
    best_HFDM = HFDM(Xh, Yh, HFDM_best_alpha_in_ADD)
    ...
    # 加入列表中
    mse_AC_in_ADD_100.append(mse_AC_in_add)
    mse_MC_in_ADD_100.append(mse_MC_in_add)
    mse_CC_in_ADD_100.append(mse_CC_in_add)
    mse_HFDM_in_ADD_100.append(mse_HFDM_in_add)

    rmse_AC_in_ADD_100.append(rmse_AC_in_add)
    rmse_MC_in_ADD_100.append(rmse_MC_in_add)
    rmse_CC_in_ADD_100.append(rmse_CC_in_add)
    rmse_HFDM_in_ADD_100.append(rmse_HFDM_in_add)

    mae_AC_in_ADD_100.append(mae_AC_in_add)
    mae_MC_in_ADD_100.append(mae_MC_in_add)
    mae_CC_in_ADD_100.append(mae_CC_in_add)
    mae_HFDM_in_ADD_100.append(mae_HFDM_in_add)

    r_squared_AC_in_ADD_100.append(r_squared_AC_in_add)
    r_squared_MC_in_ADD_100.append(r_squared_MC_in_add)
    r_squared_CC_in_ADD_100.append(r_squared_CC_in_add)
    r_squared_HFDM_in_ADD_100.append(r_squared_HFDM_in_add)

    me_AC_in_ADD_100.append(me_AC_in_add)
    me_MC_in_ADD_100.append(me_MC_in_add)
    me_CC_in_ADD_100.append(me_CC_in_add)
    me_HFDM_in_ADD_100.append(me_HFDM_in_add)

    esd_AC_in_ADD_100.append(esd_AC_in_add)
    esd_MC_in_ADD_100.append(esd_MC_in_add)
    esd_CC_in_ADD_100.append(esd_CC_in_add)
    esd_HFDM_in_ADD_100.append(esd_HFDM_in_add)
...
print("AC",np.mean(mse_AC_in_ADD_100))
print("MC",np.mean(mse_MC_in_ADD_100))
print("CC",np.mean(mse_CC_in_ADD_100))
print("HFDM",np.mean(mse_HFDM_in_ADD_100))

print("重复选取100次真值函数后每个方法的平均的rmse为:")
print("AC",np.mean(rmse_AC_in_ADD_100))
print("MC",np.mean(rmse_MC_in_ADD_100))
print("CC",np.mean(rmse_CC_in_ADD_100))
print("HFDM",np.mean(rmse_HFDM_in_ADD_100))

print("重复选取100次真值函数后每个方法的平均的mae为:")
print("AC",np.mean(mae_AC_in_ADD_100))
print("MC",np.mean(mae_MC_in_ADD_100))
print("CC",np.mean(mae_CC_in_ADD_100))
print("HFDM",np.mean(mae_HFDM_in_ADD_100))

print("重复选取100次真值函数后每个方法的平均的R²为:")
print("AC",np.mean(r_squared_AC_in_ADD_100))
print("MC",np.mean(r_squared_MC_in_ADD_100))
print("CC",np.mean(r_squared_CC_in_ADD_100))
print("HFDM",np.mean(r_squared_HFDM_in_ADD_100))

print("重复选取100次真值函数后每个方法的平均的me为:")
print("AC",np.mean(me_AC_in_ADD_100))
print("MC",np.mean(me_MC_in_ADD_100))
print("CC",np.mean(me_CC_in_ADD_100))
print("HFDM",np.mean(me_HFDM_in_ADD_100))

print("重复选取100次真值函数后每个方法的平均的误差标准差（Error standard deviation, ESD）为:")
print("AC",np.mean(esd_AC_in_ADD_100))
print("MC",np.mean(esd_MC_in_ADD_100))
print("CC",np.mean(esd_CC_in_ADD_100))
print("HFDM",np.mean(esd_HFDM_in_ADD_100))
